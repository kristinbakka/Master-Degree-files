...
isingModel <- function(rows=10,cols=6,runs=500,beta=0.5,
  ...
  if(!colsum){
    ...
    #pb=matrix(NA,nrow=1,ncol = runs+1)
    #pb= antall elementer av u[1:(runs-1),] == u[2:(runs),], delt på (rows-1)*cols
    #pb=sum((u[1:(runs-1),]-u[2:(runs),])==0)/((rows-1)*cols)
  }
  r=0 #run number
...
funValues[,1]=isingModel(rows=rows1,cols=cols1,runs=runs1,beta=beta1,
                  u=a0, colsum = TRUE)
tester=isingModel(rows=rows1,cols=cols1,runs=runs1,beta=beta1,
                   u=a0, colsum = FALSE)
testermat=matrix(tester[,6000],nrow =rows1,ncol = cols1 )
image(testermat)
#all ones (2)
