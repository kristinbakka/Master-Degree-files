...
isingModel <- function(rows=10,cols=6,runs=500,beta=0.5,
  ...
  if(!colsum){
    ...
    #k2= antall elementer av u[1:(runs-1),] == u[2:(runs),], delt på (rows-1)*cols
    k2=matrix(NA,nrow=1,ncol = runs+1)
    k2[1]= sum((u[1:(rows-1),]-u[2:(rows),])==0)/((rows-1)*cols)
    #k2[1]= ifelse((sum((u[1:(runs-1),]-u[2:(runs),])==0)/((rows-1)*cols))>(3/4),1,0)
  }
  r=0 #run number
...
#Plot after
truncate2=10000
tx2=(truncate2):(runs1+1)
truncfun=funValues[tx2,]

plot(x =tx2,y=truncfun[,1],type='l',col='red',xlim=c(truncate2,runs1+1),ylim=c(0,1),xlab='State number',ylab='Proportion of ones')
